...
| A metadata range filter (`>`, `>=`, `<`, `<=`) returns too few rows, with no error | Metadata comparisons run as **strings**: the integration renders every field as `JSON_VALUE(… RETURNING VARCHAR(1000))`, so `page_number >= 2` matches pages 2–9 but not 10–15, and `<= 12` matches 1, 10, 11, 12 | Use `in` with an explicit list of values. In SQL, filter on the zero-padded `page_label` (`BETWEEN '0004' AND '0010'`), or `CAST(… AS INT)`. See [Filtering on numbers](../README.md#filtering-on-numbers) |
| `FilterError: Operator '>=' requires a numeric value or ISO date string, got str` | The integration validates range operators before it builds the SQL, and rejects string values — so the padded `page_label` cannot be range-filtered *through Haystack* | Same fix: `in` with a list through Haystack, or `BETWEEN` in raw SQL |
