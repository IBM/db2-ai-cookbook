...
def get_men_shoes_sql(shoe_size: int):
    return f""" 
    SELECT SKU, PRODUCT_NAME, BRAND, TYPE, MATERIAL, COLOR, WEATHER_RESISTANCE, ARCH_SUPPORT, PRICE, RATING, CITY
    FROM SQ_SHOES 
    WHERE CLASS = 'Men' AND SIZE = {shoe_size}
    FETCH FIRST 5 ROWS ONLY
    """
    
# shoe_queries.py

def get_similar_shoes_sql(shoe_size: int):
    return f"""
    SELECT 
        SKU, 
        PRODUCT_NAME, 
...
def plot_similarity_mds(top_matching_vectors, my_choice_sku):
    # Parse vector strings to list of floats
    top_matching_vectors['embedding_list'] = top_matching_vectors['EMBEDDING'].apply(ast.literal_eval)
    ...
    # Run MDS for distance-preserving layout
    ...
    for i, row in top_matching_vectors.iterrows():
        ...
        if sku != my_choice_sku:
            ...
            # Annotate with just the actual distance
            mid_x = (target_point[0] + point[0]) / 2
            mid_y = (target_point[1] + point[1]) / 2
            plt.text(mid_x, mid_y, f"{true_distance:.2f}",
    ...
    plt.title(f"Distance Between Recommended Shoes and Sample Shoe {my_choice_sku} (MDS with Euclidean)")
    plt.tight_layout()
    plt.show()